...
class Federated_Learner:
	...
	def __init__(self, args, data_prepper):
		...
		self.reputation_threshold_coef = self.args['reputation_threshold_coef'] if 'reputation_threshold_coef' in self.args else 1.0/6.0 
		self.mem_tmc = np.zeros((0, self.n_participants))#dsq:
	...
	def train(self):
		...
		for epoch in range(fl_epochs):
			...
			#self.reputations, self.reputation_threshold, self.R  = compute_reputations_shapley(self,self.federated_model, self.filtered_updates,self.reputations, self.reputation_threshold, self.R, reputation_fade=self.reputation_fade, split=self.args['split'], reputation_threshold_coef=self.reputation_threshold_coef, eta=1)
			#self.reputations_pretrain, self.reputation_threshold_pretrain, self.R_pretrain  = compute_reputations_shapley(self,self.federated_model_pretrain, self.filtered_updates_pretrain,self.reputations_pretrain, self.reputation_threshold_pretrain, self.R_pretrain, reputation_fade=self.reputation_fade, split=self.args['split'], reputation_threshold_coef=self.reputation_threshold_coef,eta=1)
      #dsq:深拷贝的问题
			self.mem_tmc = np.zeros((0, self.n_participants))#dsq:
			reputations_temp, reputation_threshold_temp, R_temp  = compute_reputations_shapley(self,self.federated_model, self.filtered_updates,self.reputations, self.reputation_threshold, self.R, reputation_fade=self.reputation_fade, reputation_threshold_coef=self.reputation_threshold_coef)
			self.mem_tmc = np.zeros((0, self.n_participants))#dsq:
			reputations_pretrain_temp, reputation_threshold_pretrain_temp, R_pretrain_temp  = compute_reputations_shapley(self,self.federated_model_pretrain, self.filtered_updates_pretrain,self.reputations_pretrain, self.reputation_threshold_pretrain, self.R_pretrain, reputation_fade=self.reputation_fade, reputation_threshold_coef=self.reputation_threshold_coef)
			...
			self.reputations = copy.deepcopy(reputations_temp)
			self.reputation_threshold = copy.deepcopy(reputation_threshold_temp)
			self.R = copy.deepcopy(R_temp)
			self.reputations_pretrain = copy.deepcopy(reputations_pretrain_temp)
			self.reputation_threshold_pretrain = copy.deepcopy(reputation_threshold_pretrain_temp)
			self.R_pretrain = copy.deepcopy(R_pretrain_temp)
	...
	def update_reputations(self, participant_val_accs, participant_val_accs_pretrain):
		self.reputations, self.reputation_threshold, self.R  = compute_reputations_shapley(self,self.federated_model, self.filtered_updates,self.reputations, self.reputation_threshold, self.R, reputation_fade=self.reputaion_fade, split=self.args['split'], reputation_threshold_coef=self.reputation_threshold_coef, eta=1)
		self.reputations_pretrain, self.reputation_threshold_pretrain, self.R_pretrain  = compute_reputations_shapley(self,self.federated_model_pretrain, self.filtered_updates_pretrain,self.reputations_pretrain, self.reputation_threshold_pretrain, self.R_pretrain, reputation_fade=self.reputation_fade, split=self.args['split'], reputation_threshold_coef=self.reputation_threshold_coef,eta=1)
        #self.reputations, self.reputation_threshold, self.R  = compute_reputations_sinh(self.reputations, self.reputation_thresholdself.reputation_threshold, self.R, participant_val_accs, alpha=self.args['alpha'], split=self.args['split'], reputation_threshold_coef=self.reputation_threshold_coef)
""".............................dsq's reputations........................."""
#def compute_reputations_shapley(reputations, reputation_threshold, R, val_accs, alpha=5, reputation_fade=1, split='powerlaw', reputation_threshold_coef=1.0/3.0):

def compute_reputations_shapley(self,now_federated_model, now_filtered_updates,reputations,reputation_threshold, R, reputation_fade=1, reputation_threshold_coef=1.0/6.0):
    #print("______________________每次计算shaplely值之前the reputations_all________________________________{}".format(np.around(reputations.tolist(), 3)))
    #print("______________________每次计算shaplely值之前R________________________________{}".format(R))
    R_size = len(R)
    reputations_epoch = compute_tmc_shapley(self, R, now_filtered_updates, now_federated_model, Max_num_sequences=50)
    for i in R:
        if reputation_fade == 1:
           reputations[i] = reputations[i] * 0.8 + reputations_epoch[i] * 0.2
        else:
           reputations[i] = (reputations[i] + reputations_epoch[i]) * 0.5
    #print("______________________the reputations_all________________________________{}".format(np.around(reputations.tolist(), 3)))    
	# normalize among the reputable participants
    #先不做归一化处理
    reputations /= reputations.sum().float()
    print("______________________归一化后the reputations_all________________________________{}".format(np.around(reputations.tolist(), 3)))    
    # update reputable participants
    #R = [i for i in R if reputations[i] >= reputation_threshold]
    R = [i for i in R if reputations[i] >= 0]
    ...
    for i in range(len(reputations)):
        if reputations[i] < reputation_threshold:
            reputations[i] = 0
    #print("______________________去除负数后R________________________________{}".format(R))
    
    if R_size != len(R):
        # normalize among the reputable participants
        reputations /= reputations.sum().float()
        #print("______________________去除最小后the reputations_all________________________________{}".format(np.around(reputations.tolist(), 3)))
        #reputation_threshold = compute_reputation_threshold(len(R), self.split, reputation_threshold_coef)
...
""".............................dsq's reputations........................."""
def compute_tmc_shapley(self, R, grad_updates, federated_model, Max_num_sequences=50,err=0.001):
	...
	test_loss_prev, test_acc_prev = evaluate(federated_model, self.test_loader, self.device, verbose=False)
	prev_contribution = test_acc_prev.data
	
	num_sequence = 0
	for sequence in all_sequences:
		
		my_error=error(self.mem_tmc)
		print("my_error",my_error)
		if my_error< err:
			break
		#if ((num_sequence+1)%100) == 0:
		#	self.mem_tmc = np.zeros((0, self.n_participants))#dsq:
		...
		for participant_id in sequence:
			if participant_id in R:
				if self.args['aggregate_mode'] == 'sum':
					...
					weight = marginal_contributions[participant_id]
				else: # default average
					if self.split != 'classimbalance':
						weight = self.shard_sizes[participant_id] * 1. / sum(self.shard_sizes)
					else:
						assert self.args['dataset'] in ['mnist', 'cifar10'], "Fedavg and classimbalance Not supported for this dataset {}".format(self.args['dataset'])
						# currently only for cifar10 and mnist, so a total of 10 classes
						n_classes = 10
						class_sizes = np.linspace(1, n_classes, self.n_participants, dtype='int')
						weight = class_sizes[participant_id] / n_classes
                
				running_model = add_update_to_model(running_model, grad_updates[participant_id],weight)
				test_loss, test_acc = evaluate(running_model, self.test_loader, self.device, verbose=False)
		...
		self.mem_tmc = np.concatenate([
                self.mem_tmc, 
                np.reshape(marginal_contributions, (1,-1))
            ])
	np.set_printoptions(threshold = np.inf)		
	print("所有的康康",self.mem_tmc)

	return (np.cumsum(self.mem_tmc, 0)[-1]) / num_sequence
"""
	all_vals = (np.cumsum(mem, 0)/np.reshape(np.arange(1, len(mem)+1), (-1,1)))[-100:]#算了那个迭代的式子
...
def error(mem): 
	...
	all_vals = (np.cumsum(mem, 0)/np.reshape(np.arange(1, len(mem)+1), (-1,1)))[-100:]#算了那个迭代的式子
	errors = np.mean(np.abs(all_vals[-100:] - all_vals[-1:])/(np.abs(all_vals[-1:]) + 1e-12), -1)
	print("max_errors:",np.max(errors))
	return np.max(errors)  

def compute_reputations_sinh(reputations, reputation_threshold, R, val_accs, alpha=5, reputation_fade=1, split='powerlaw', reputation_threshold_coef=1.0/6.0):
...
def compute_reputation_threshold(R_size, split, coef=1.0/6.0):
